...
int main(void) {
    ...
    unsigned char* move; // the move on the host CPU machine
    unsigned char* dev_move; // the move for the GPU device

    unsigned char* results;
    unsigned char* dev_results;
    unsigned char* moves;
    unsigned char* dev_moves;
    ...
    while(isNotEnded(nim) && a == 0) {
        ...
        cudaHandleError( cudaMalloc( (void**)&dev_move, sizeof(unsigned char) ) );
        ...
        // Copy nim to the GPU
        cudaHandleError( cudaMemcpy( dev_results, results, maxMoves * sizeof(unsigned char), cudaMemcpyHostToDevice ) );
        ...
        GPU_minmax<<<grid, thread>>>(nim, NUM_ROWS, dev_results, dev_moves, numMoves, dev_move);
        
        cudaHandleError( cudaPeekAtLastError() );

        // Copy the move back from the GPU to the CPU
        cudaHandleError( cudaMemcpy( move, dev_move, sizeof(unsigned char), cudaMemcpyDeviceToHost ) );
